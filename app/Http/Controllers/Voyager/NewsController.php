...
class NewsController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($orderBy && (in_array($orderBy, $dataType->fields()) || !empty($row))) {
                ...
                if(isset($user->lri->id))
                    $dataTypeContent = call_user_func([$query->where('lri_type_id', $user->lri->id)->latest($model::CREATED_AT), $getter]);
                else   
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'institution', 'user'));
    }
